...
gpg --list-secret-keys --with-colons --fingerprint "$ID" | grep fpr | cut -d : -f 10 | xargs gpg --batch --yes --delete-secret-keys
gpg --list-keys --with-colons --fingerprint "$ID" | grep fpr | cut -d : -f 10 | xargs gpg --batch --yes --delete-keys
